...
System.getInitialProps = wrapper.getInitialPageProps((store) => async () => {
  let option = [];
  await CallApi.ExecuteApi(
    store.getState().system.factory.name,
    store.getState().system.factory.ip + "/system/get_system_type_list",
    {}
  )
    .then((res) => {
      if (res) {
  ...
  return {
    system_Type_Options: option,
  };
});

function System({ system_Type_Options }) {
  return (
    <ComponentProvider>
      <ProgramProvider>
        <System_Content system_Type_Options={system_Type_Options} />
      </ProgramProvider>
    </ComponentProvider>
  );
}

function System_Content({ system_Type_Options }) {
